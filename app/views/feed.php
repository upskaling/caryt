...
try {
    ...
    if (isset($_POST['delete'])) {
        ...
        $query = $pdo->prepare('DELETE FROM "admin_feed"
        WHERE (("rowid" = :id));');
    ...
    if (isset($_POST['xmlUrl'])) {
        ...
        $query = $pdo->prepare('UPDATE "admin_feed" SET "xmlurl" = :xmlurl, "siteurl" = :siteurl, "title" = :title, "update_interval" = :update_interval, "category" = :category, "mute" = :mute
        WHERE "rowid" = :id
        LIMIT :id;');
        $query->execute([
            'xmlurl' => filter_var($_POST['xmlUrl'], FILTER_VALIDATE_URL),
    ...
    $query =  $pdo->prepare('SELECT * FROM "admin_feed"
    WHERE "rowid" = :id');
    $query->execute([
        'id' => $_GET['id']
    ]);
    ...
    $query =  $pdo->prepare('SELECT *, "rowid" FROM "admin_category"');
    $query->execute();

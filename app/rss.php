...
$channel->appendChild($dom->createElement('link', $GLOBALS['config']['url'] . 'index2.xml'));
...
$youtubr_dl_wl_dir = glob($GLOBALS['config']['YOUTUBR_DL_WL'] . '/*/');
...
foreach (scandir($dir_video) as $name) {
  ...
  if (is_file($dir_video . $name . '/' . $name . '.' . $extension_thumbnail)) {
    $jsonb['thumbnail'] = $GLOBALS['config']['url'] . 'f.php?' .  $dir_date . '/' . $name . '.' . $extension_thumbnail;
  }
  ...
  if (is_file($jsonb['_filename'])) {
    $url_video = $GLOBALS['config']['url'] . 'f.php?' . $dir_date . '/' . basename($jsonb['_filename']);
